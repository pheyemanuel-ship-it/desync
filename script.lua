...
keekhub.Name = "keekhub"
keekhub.ZIndexBehavior = Enum.ZIndexBehavior.Sibling
keekhub.Parent = game.Players.LocalPlayer.PlayerGui
...
Main.InputBegan:Connect(function(input)
    if input.UserInputType == Enum.UserInputType.MouseButton1 then
        dragging = true   -- ← must be TRUE to start dragging
...
UserInputService.InputEnded:Connect(function(input)
    if input.UserInputType == Enum.UserInputType.MouseButton1 then
        dragging = false -- ← stop dragging when mouse released
    end
end))
Main.InputChanged:Connect(function(input)
if input.UserInputType == Enum.UserInputType.MouseMovement then
...
if channel then
channel:DisplaySystemMessage("[keekhub] Authenticated successfully.")
end
end)
print("[keekhub] Loaded successfully.")
